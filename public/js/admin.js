...
const toggleMicroButton = document.querySelector("#toggle-micro");
const endSessionButton = document.querySelector('#end-session');

endSessionButton.addEventListener("click", endSession)
toggleMicroButton.addEventListener("click", toggleMicro)
...
function getStream(muteMic = false) {
    if (window.stream) {
        window.stream.getTracks().forEach(track => {
            track.stop();
        });
    }
    console.log(muteMic?null:audioSelect.value);
    const audioSource = muteMic?null:audioSelect.value;
